...
- [ ] [F04](./04-public-html-expansion.md) — In progress: homepage, hubs and catalog-driven service/package rendering implemented locally; latest increment awaits deployment/manual acceptance
...
**Recommended next task:** deploy/verify [F04 catalog-driven rendering](./04-public-html-expansion.md) and the [F13 Booking fix](./13-performance-and-core-web-vitals.md). Run both deployed smoke tools, check CMS publication/rebuild behavior and repeat Booking performance measurements. After F04 acceptance, continue gallery and remaining core public families. Totals are **10 / 19 complete** for implementation tracking; deployment/external follow-ups remain recorded separately.
...
|---|---|---|
| Rendering — F04 | Deploy/verify catalog-driven service/package rendering, then gallery and subsequent core families | New eligible CMS routes must render after rebuild without code edits; retain publication and manual acceptance gates |
