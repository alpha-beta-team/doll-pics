    ...
    <>
      ...
      <label className="relative hidden shrink-0 sm:block">
        ...
        <select
          ...
          value={value}
          onChange={event => onChange(event.target.value as BookingSort)}
          className="h-11 min-w-52 appearance-none rounded-lg border border-admin-control bg-admin-surface py-0 pl-9 pr-8 text-sm font-semibold text-admin-secondary outline-none transition hover:bg-admin-muted hover:text-admin-text focus-visible:ring-2 focus-visible:ring-admin-focus"
        >
          {BOOKING_SORT_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
        </select>
