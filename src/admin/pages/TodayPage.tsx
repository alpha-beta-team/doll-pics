...
export function TodayPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    <AdminPageHeader eyebrow={new Intl.DateTimeFormat('en-IN', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date())} title="Today’s work" description="Start at the top and finish one item at a time." actions={<><InstallAppButton /><AdminIconButton label="Refresh today’s work" onClick={() => void load()}><RefreshCw className="h-4 w-4" /></AdminIconButton><AdminButton onClick={() => navigate('/admin/enquiries?new=1')} className="hidden sm:inline-flex"><Plus className="h-4 w-4" />Add enquiry</AdminButton></>} />
