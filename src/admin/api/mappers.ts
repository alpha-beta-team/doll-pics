...
export function mapEnquiry(doc: Record<string, unknown>): Enquiry {
  ...
  return {
    ...
    lastFollowUpCompletedAt: doc.lastFollowUpCompletedAt
      ? String(doc.lastFollowUpCompletedAt)
      : undefined,
    convertedBookingId: doc.convertedBookingId ? String(doc.convertedBookingId) : undefined,
    whatsappOptIn: doc.whatsappOptIn === true,
    whatsappOptInAt: doc.whatsappOptInAt ? String(doc.whatsappOptInAt) : undefined,
    whatsappOptInSource: String(doc.whatsappOptInSource ?? ''),
    whatsappNotificationsEnabled: doc.whatsappNotificationsEnabled === true,
    whatsappOptOutAt: doc.whatsappOptOutAt ? String(doc.whatsappOptOutAt) : undefined,
    preferredLanguage: String(doc.preferredLanguage ?? 'en'),
    createdAt: (doc.createdAt as string) ?? new Date().toISOString(),
...
export function mapBooking(doc: Record<string, unknown>): Booking {
  ...
  return {
    ...
    startTime: String(doc.startTime ?? ''),
    endTime: String(doc.endTime ?? ''),
    durationMinutes: doc.durationMinutes == null ? null : Number(doc.durationMinutes),
    durationHours: (doc.durationHours as number) ?? 0,
    location: (doc.location as string) ?? '',
    paymentDueDate: (doc.paymentDueDate as string) || String(doc.reminderDate ?? '').slice(0, 10),
    nextFollowUpAt: doc.nextFollowUpAt ? String(doc.nextFollowUpAt) : undefined,
    followUpNote: (doc.followUpNote as string) ?? '',
    lastFollowUpCompletedAt: doc.lastFollowUpCompletedAt ? String(doc.lastFollowUpCompletedAt) : undefined,
    notes: (doc.notes as string) ?? '',
    ...
    payments: rawPayments.map(payment => ({
      ...
      method: (payment.method as Booking['payments'][number]['method']) ?? 'other',
      reference: String(payment.reference ?? ''),
      note: String(payment.note ?? ''),
      createdAt: payment.createdAt ? String(payment.createdAt) : undefined,
    })),
    paymentSummary: {
      amountPaid: Number(rawSummary.amountPaid) || 0,
      balanceDue: rawSummary.balanceDue == null ? null : Number(rawSummary.balanceDue),
      status: (rawSummary.status as Booking['paymentSummary']['status']) ?? 'unpriced',
    },
