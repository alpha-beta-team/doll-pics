...
function mapOrderedItem<
  T extends { id: string; order: number; isPublished: boolean },
>(
  ...
  return {
    ...
    update: async (id: string, data: Record<string, unknown>): Promise<T> => {
      const payload = Object.fromEntries(
        Object.entries(data).filter(([field]) => field !== "id"),
      );
...
export const api = {
  ...
  async uploadTeamMemberImage(file: File, transform: ImageTransform | null = null): Promise<{ url: string; originalUrl: string; storageKey: string; imageTransform: ImageTransform | null }> {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('imageTransform', JSON.stringify(transform));
    return request<{ url: string; originalUrl: string; storageKey: string; imageTransform: ImageTransform | null }>('/admin/media/team-member', {
      method: 'POST',
      auth: true,
      body: formData,
    });
  ...
  async updateTeamMemberImage(id: string, transform: ImageTransform | null): Promise<{ url: string; originalUrl: string; storageKey: string; imageTransform: ImageTransform | null }> {
    return request<{ url: string; originalUrl: string; storageKey: string; imageTransform: ImageTransform | null }>(`/admin/team-members/${id}/image`, {
      method: 'PATCH',
      auth: true,
      body: JSON.stringify({ imageTransform: transform }),
    });
  ...
  createStoryScene: (data: Omit<StoryScene, "id">) =>
    storyScenesApi.create(data),
  updateStoryScene: (id: string, data: Partial<StoryScene>) =>
    storyScenesApi.update(id, data),
  ...
  createBehindScene: (data: Omit<BehindScene, "id">) =>
    behindScenesApi.create(data),
  updateBehindScene: (id: string, data: Partial<BehindScene>) =>
    behindScenesApi.update(id, data),
