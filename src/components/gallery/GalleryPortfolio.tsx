...
function GalleryLightbox({
  ...
  const [useGridSource, setUseGridSource] = useState(false);
  ...
  const handleTouchEnd = (event: TouchEvent) => {
    ...
    if (Math.abs(xDistance) < 48 || Math.abs(xDistance) <= Math.abs(yDistance)) return;
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[1200] touch-pan-y bg-black/95 backdrop-blur-md"
    >
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-3 pb-24 pt-16 md:px-20 md:pb-16">
        <img
          key={photo.id}
          src={useGridSource ? photo.sources.src : photo.lightboxSrc}
          alt={photo.sources.alt}
          width={photo.width}
          height={photo.height}
          ...
          decoding="async"
          onError={() => {
            if (!useGridSource && photo.sources.src !== photo.lightboxSrc) {
              setUseGridSource(true);
            }
          }}
        />
      </div>
      ...
      <div className="sr-only" aria-live="polite">
        Photograph {index + 1} of {photos.length}: {photo.title}
      </div>
    </div>
  );
}
