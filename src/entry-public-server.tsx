...
export async function renderPublicPage(input: {
  ...
  const html = renderToString(<StrictMode><StaticRouter location={snapshot.path}><AppRoutes snapshot={snapshot} PilotPage={input.path === '/' ? Site : input.path === '/services' ? ServicesHub : input.path === '/packages' ? Packages : packagePage ? PackageCategoryPage : ServicePage} /></StaticRouter></StrictMode>);
